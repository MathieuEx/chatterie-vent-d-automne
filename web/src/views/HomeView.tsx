...
const FALLBACK = {
  fr: {
    ...
    noCats: "Nos reproducteurs seront bientôt présentés ici.",
    emptyGallery:
      "Galerie à venir — les premières photos seront ajoutées dès la prochaine portée.",
    galleryAlt: "Photo de chaton",
  },
  en: {
    title: "La Chatterie des Vents d'Automne | Ragdoll Cattery in Toulouse",
    description:
    ...
    noCats: "Our breeding cats will be introduced here shortly.",
    emptyGallery: "Gallery coming soon — the first photos will be added with the next litter.",
    galleryAlt: "Kitten photo",
  },
} as const;
...
export default async function HomeView({ locale }: { locale: Locale }) {
  ...
  const [rawLatestLitter, galleryImages, rawSettings, rawCats, testimonials, rawHomePage] =
    await Promise.all([
      getLatestLitter(),
      getGalleryImages(6),
      getSiteSettings(),
  ...
  const latestLitter = localize(rawLatestLitter, locale);
  const siteSettings = localize(rawSettings, locale);
  const homePage = localize(rawHomePage, locale);
        ...
        <div className="container">
          ...
          >
            ...
            <Link href={route("cats", locale)} className="btn-secondary">
              {getDictionary(locale).cat.seeAll}
            </Link>
          </div>
          ...
          {featuredCats.length > 0 ? (
            ...
            <p className="body-text-sm">{fallback.noCats}</p>
          )}
        </div>
      </section>
        ...
        <div className="container">
          ...
          {galleryImages.length > 0 ? (
            ...
            <p className="body-text-sm" style={{ marginTop: "2rem" }}>
              {gallerySection?.emptyStateText ?? fallback.emptyGallery}
            </p>
          )}
        </div>
      </section>
          ...
          <div className="container">
            ...
            <div style={{ maxWidth: 360 }}>
              <LitterCard
                litter={latestLitter}
                locale={locale}
                price={litterPrice(latestLitter, siteSettings, locale)}
              />
            </div>
            <div style={{ marginTop: "2rem" }}>
              <Link href={route("kittens", locale)} className="btn-secondary">
                {getDictionary(locale).litter.seeAll}
              </Link>
            </div>
          </div>
        </section>
